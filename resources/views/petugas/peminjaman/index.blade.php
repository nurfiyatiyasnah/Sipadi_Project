                                ...
                                <div class="inline-flex items-center justify-center gap-3">
                                    ...
                                    @if(in_array($status, ['menunggu', 'pending', 'pengajuan', 'diajukan']))
                                        ...
                                        <div x-data="{ showRejectModal: false }" class="inline">
                                            <button @click="showRejectModal = true" type="button" class="text-rose-500 hover:text-rose-700 transition" title="Tolak Pengajuan">
                                            ...
                                            <div x-show="showRejectModal" style="display: none;" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
                                                <div x-show="showRejectModal" x-transition.opacity class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm transition-opacity"></div>
                                                            ...
                                                            <div class="sm:flex sm:items-start">
                                                                ...
                                                                <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                                                                    ...
                                                                    <div class="mt-3 text-sm text-slate-500 space-y-2">
                                                                        <p>Apakah Anda yakin ingin menolak pengajuan peminjaman untuk <strong>{{ $p->buku?->judul }}</strong> oleh <strong>{{ $p->anggota?->nama_anggota }}</strong>?</p>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            
                                                            <form action="{{ route('petugas.peminjaman.tolak', $p->id_peminjaman) }}" method="POST" id="form-reject-{{ $p->id_peminjaman }}">
                                                                @csrf
                                                                <div class="mt-4">
                                                                    <label for="catatan_admin_{{ $p->id_peminjaman }}" class="block text-sm font-semibold text-slate-700 text-left mb-2">Alasan Penolakan (Opsional)</label>
                                                                    <textarea name="catatan_admin" id="catatan_admin_{{ $p->id_peminjaman }}" rows="3" class="w-full rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm focus:border-rose-500 focus:ring-rose-500 outline-none" placeholder="Tuliskan alasan mengapa pengajuan ini ditolak..."></textarea>
                                                                </div>
                                                            </form>
                                                        </div>
                                                        <div class="bg-slate-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                                                            <button type="submit" form="form-reject-{{ $p->id_peminjaman }}" class="inline-flex w-full justify-center rounded-lg bg-rose-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-rose-500 sm:ml-3 sm:w-auto transition">
                                                                Ya, Tolak
                                                            </button>
                                                            <button @click="showRejectModal = false" type="button" class="mt-3 inline-flex w-full justify-center rounded-lg bg-white px-4 py-2 text-sm font-semibold text-slate-900 shadow-sm ring-1 ring-inset ring-slate-300 hover:bg-slate-50 sm:mt-0 sm:w-auto transition">
                                                                Batal
                                                            </button>
                                                        </div>
                                                    </div>
