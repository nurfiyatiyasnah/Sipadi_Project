...
@section('content')
<div class="max-w-7xl mx-auto px-6 lg:px-12 py-8 space-y-8" x-data="{ showQr: false }">
    
    <!-- Page Header -->
    <div class="border-b border-slate-200/60 pb-5">
        <h1 class="font-serif text-3xl font-bold text-[#04241e]">Profil Anggota</h1>
    ...
    <!-- Two Column Grid -->
    <div class="grid gap-8 lg:grid-cols-[0.9fr_1.6fr] items-start">
        
        <!-- Left Column: Photo & Membership Card -->
        <div class="space-y-6">
            
            <!-- Profile Info Card -->
            <div class="bg-white border border-slate-100 rounded-3xl p-8 shadow-sm text-center relative overflow-hidden">
                ...
                <!-- Name & Details -->
                <h2 class="text-xl font-bold text-[#061b3a] mt-5 leading-snug">{{ $anggota->nama_lengkap }}</h2>
                <p class="text-xs text-slate-400 mt-1 font-semibold">{{ $anggota->kalangan ?? 'Anggota Perpustakaan' }}</p>
            ...
            <div class="bg-[#04241e] text-white p-8 rounded-3xl relative overflow-hidden shadow-xl shadow-[#04241e]/10 flex flex-col justify-between h-[200px]">
                ...
                <div class="flex justify-between items-start relative z-10">
                    <div>
                        ...
                        <p class="text-2xl font-mono font-bold tracking-wide mt-1.5">{{ $anggota->no_anggota }}</p>
                    </div>
            ...
            <div class="bg-white border border-slate-100 rounded-3xl p-8 shadow-sm space-y-6">
                ...
                <div class="grid gap-6 md:grid-cols-2 text-sm">
                    <div>
                        ...
                        <p class="text-slate-800 font-bold mt-1">{{ $anggota->nama_lengkap }}</p>
                    </div>

                    <div>
                        <p class="text-slate-400 font-semibold text-xs">Nomor Induk Kependudukan (NIK)</p>
                        <p class="text-slate-800 font-bold mt-1 font-mono">{{ $anggota->nik }}</p>
                    </div>
                    ...
                    <div>
                        ...
                        <p class="text-slate-800 font-bold mt-1">{{ $anggota->jenis_kelamin ?? 'Perempuan' }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white border border-slate-100 rounded-3xl p-8 shadow-sm space-y-6">
                ...
                <div class="grid gap-6 md:grid-cols-2 text-sm">
                    <div>
                        ...
                        <p class="text-slate-800 font-bold mt-1 truncate" title="{{ $user->email }}">{{ $user->email }}</p>
                    </div>

                    <div>
                        <p class="text-slate-400 font-semibold text-xs">Nomor Telepon / WhatsApp</p>
                        <p class="text-slate-800 font-bold mt-1 font-mono">{{ $anggota->no_telepon ?? '-' }}</p>
                    </div>

                    <div class="md:col-span-2">
                        <p class="text-slate-400 font-semibold text-xs">Alamat Sesuai KTP</p>
                        <p class="text-slate-800 font-bold mt-1 leading-relaxed">{{ $anggota->alamat ?? '-' }}</p>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white w-full max-w-sm rounded-3xl p-8 shadow-2xl relative" @click.outside="showQr = false">
            ...
            <div class="bg-slate-50 border border-dashed border-slate-200 rounded-2xl p-6 flex flex-col items-center justify-center space-y-4">
                ...
                <div class="text-center">
                    ...
                    <p class="font-mono text-sm font-bold text-slate-700 mt-0.5">{{ $anggota->no_anggota }}</p>
                </div>
            </div>
