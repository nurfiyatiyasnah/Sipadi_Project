<div class="mx-auto max-w-[1180px] space-y-6">
    ...
    <nav class="flex text-sm text-slate-500 gap-2">
        ...
        <span>&rsaquo;</span>
        <span class="text-slate-800 font-semibold">Edit Buku</span>
    </nav>

    {{-- Header --}}
    <div>
        <h2 class="font-serif text-3xl font-bold text-[#071426]">Edit Buku</h2>
        <p class="text-sm text-slate-500 mt-1">Perbarui informasi buku yang sudah terdaftar.</p>
    </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="relative flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50/50 p-6 text-center hover:bg-slate-50 transition cursor-pointer">
                        ...
                        @elseif ($existing_cover)
                            <div class="h-48 w-36 overflow-hidden rounded-xl bg-slate-100 border border-slate-200 shadow-sm relative">
                                <img src="{{ Str::startsWith($existing_cover, 'http') ? $existing_cover : Storage::url($existing_cover) }}" class="h-full w-full object-cover">
                            </div>
                            <p class="mt-2 text-xs font-semibold text-slate-500">Cover Saat Ini</p>
                        @else
